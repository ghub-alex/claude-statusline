...
def render_location(data):
    ...
    status_bits = []
    if staged:
        status_bits.append(f"{COLORS['green']}+{staged}{WHITE}")
    if modified:
        status_bits.append(f"{COLORS['yellow']}~{modified}{WHITE}")
    status = " " + " ".join(status_bits) if status_bits else ""
    return f"{dirname} ({COLORS['red']}{branch}{WHITE}{status})"
...
def render_segments(data):
    ...
    if five_pct is None:
        ...
        segments.append(f"5h {color}{five_pct:.0f}%{WHITE}{suffix}")
    ...
    if week_pct is None:
        ...
        suffix = f" (resets {reset_str})" if reset_str else ""
        segments.append(f"7d {color}{week_pct:.0f}%{WHITE}{suffix}")

    effort = data.get("effort", {}).get("level")
    if effort:
        segments.append(f"effort: {effort}")
...
def main():
    ...
    parts = [f"{COLORS['cyan']}[{model}]{WHITE}", render_location(data)]
    parts.extend(render_segments(data))

    print(WHITE + " | ".join(parts) + RESET)
